...
class Pemesanan
{
    ...
    public function save()
    {
        ...
        if (isset($this->id)) {
            ...
            return $stmt->execute([$this->UserID, $this->VaksinID, $this->Schedule, $this->Location, $this->EWalletID, $this->IsConfirm ? 1 : 0, $this->id]);
        } else {
            $stmt = $db->prepare("INSERT INTO Pemesanan (UserID, VaksinID, Schedule, Location, EWalletID, IsConfirm) VALUES (?, ?, ?, ?, ?, ?)");
            return $stmt->execute([$this->UserID, $this->VaksinID, $this->Schedule, $this->Location, $this->EWalletID, $this->IsConfirm ? 1 : 0]);
        }
    }
